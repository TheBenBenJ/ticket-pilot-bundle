...
final class Configuration implements ConfigurationInterface
{
    ...
    private function addReviewSection(\Symfony\Component\Config\Definition\Builder\ArrayNodeDefinition $root): void
    {
        $root->children()->arrayNode('review')->canBeEnabled()->children()
            ...
            ->end()
            ->scalarNode('scenarios_dir')->defaultValue('.ticket-pilot/scenarios')
                ->info('[agent driver] Directory (relative to the project) where the executed review scenario is saved as Markdown.')
            ->end()
            ->booleanNode('write_scenario')->defaultTrue()
                ->info('[agent driver] Persist the scenario block the agent emits after each review.')
            ->end()
            ->scalarNode('scenario_start_marker')->defaultValue('<<<REVIEW_SCENARIO')
                ->info('[agent driver] Opening marker of the scenario block the agent must emit.')
            ->end()
            ->scalarNode('scenario_end_marker')->defaultValue('REVIEW_SCENARIO>>>')
                ->info('[agent driver] Closing marker of the scenario block.')
            ->end()
    ...
    private function addSourcesSection(\Symfony\Component\Config\Definition\Builder\ArrayNodeDefinition $root): void
    {
        $root->children()->arrayNode('sources')->addDefaultsIfNotSet()->children()
            ->arrayNode('jira')->canBeEnabled()->children()
                ...
                ->scalarNode('status_after_merge_request')->defaultValue('')
                    ->info('Jira status name to transition to after a merge request is opened (empty = skip).')
                ->end()
                ->scalarNode('status_after_review_passed')->defaultValue('')
                    ->info('Jira status after a passed agent/recipe review (empty = skip).')
                ->end()
                ->scalarNode('status_after_review_failed')->defaultValue('')
                    ->info('Jira status after a failed review (empty = skip).')
                ->end()
                ->scalarNode('status_after_review_inconclusive')->defaultValue('')
                    ->info('Jira status after an inconclusive review (empty = uses status_after_review_failed).')
                ->end()
                ->scalarNode('status_after_iterate')->defaultValue('')
                    ->info('Jira status after ia:iterate completes (empty = skip).')
                ->end()
            ->end()->end()
    ...
    private function addAgentsSection(\Symfony\Component\Config\Definition\Builder\ArrayNodeDefinition $root): void
    {
        $root->children()->arrayNode('agents')->addDefaultsIfNotSet()->children()
            ->arrayNode('cursor')->canBeDisabled()->children()
                ...
                ->arrayNode('models')
                    ->scalarPrototype()->end()
                    ->defaultValue(['auto', 'composer-1', 'claude-4-sonnet', 'claude-4-sonnet-thinking', 'claude-4-opus', 'gpt-4o', 'o3', 'gemini-2.5-pro'])
                    ->info('Fallback/merged model list for the dashboard when "agent models" cannot be queried live.')
                ->end()
            ->end()->end()
