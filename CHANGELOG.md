...
### Fixed
- **Review screenshots showed as "not web-served" (empty screenshot dirs).** The ingest decoded
  `_files` payloads with strict base64, which silently dropped any screenshot sent as a
  `data:` URI or containing whitespace/newlines — creating an empty `public/ticket-pilot/screenshots/<runId>/`
  directory and leaving the dashboard with bare names. `RunIngestController` now decodes
  tolerantly (strips a `data:` prefix, removes whitespace, non-strict decode), so the files are
  actually written and served as `<img>` previews regardless of how the CI bundle version
  encoded them.

### Internal
- Cleared 3 long-standing static-analysis nits (missing array value types, redundant `?? ''`,
  dead `data:` branch) flagged across the recent screenshot work.
...
### Changed
- **Dashboard screenshot preview.** Review thumbnails open in an inline lightbox
  (prev/next, Escape to close) instead of forcing a new browser tab. A discreet
  « Open in new tab » link remains inside the viewer.
